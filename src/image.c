...
int image_tint(XImage *image, XColor *color, CARD8 alpha)
{
  ...
  case 32:
    return image_tint_32((CARD8 *)image->data, image->width * image->height,
        color->pixel, alpha);
  case 24:
    return image_tint_24((CARD8 *)image->data, image->width * image->height,
        color->pixel, alpha);
  case 16:
    return image_tint_16((CARD16 *)image->data, image->width * image->height,
        color->pixel, alpha);
  case 15:
    return image_tint_15((CARD16 *)image->data, image->width * image->height,
        color->pixel, alpha);
  default:
...
int image_compose(XImage *image, XImage *bg, CARD8 *mask)
{
  ...
  case 24:
    return image_compose_24((CARD8 *)image->data, (CARD8 *)bg->data, mask,
        (image->width * image->height));
  case 16:
    return image_compose_16((CARD16 *)image->data, (CARD16 *)bg->data, mask,
        (image->width * image->height));
  case 15:
    return image_compose_15((CARD16 *)image->data, (CARD16 *)bg->data, mask,
        (image->width * image->height));
  default:
