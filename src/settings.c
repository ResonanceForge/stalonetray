...
int parse_scrollbars_highlight_color(
    int, const char **argv, size_t struct_offset, int)
{
  char **highlight_color = REF(char *);

  free(*highlight_color);
  *highlight_color =
      strcasecmp(argv[0], "disable") == 0 ? NULL : xstrdup(argv[0]);
  return SUCCESS;
}
...
int parse_ignored_classes(
    int argc, const char **argv, size_t struct_offset, int)
{
...
int parse_gravity(int, const char **argv, size_t struct_offset, int silent)
{
  ...
  if (strlen(gravity_s) > 2) goto fail;
  ...
  if ((parsed & GRAV_N && parsed & GRAV_S)
      || (parsed & GRAV_E && parsed & GRAV_W))
    goto fail;
...
int parse_strut_mode(int, const char **argv, size_t struct_offset, int silent)
{
  ...
  if (!strcasecmp(argv[0], "auto"))
    ...
    *strut_mode = WM_STRUT_NONE;
    ...
    PARSING_ERROR("one of top, bottom, left, right, or auto expected", argv[0]);
    return FAILURE;
  }
...
int parse_bool_rev(
    int argc, const char **argv, size_t struct_offset, int silent)
{
  int *boolean = REF(int);
...
int parse_remote_click_type(
    int, const char **argv, size_t struct_offset, int silent)
{
...
int parse_size(int, const char **argv, size_t struct_offset, int)
{
  ...
  if (bitmask == 0 || bitmask & ~(WidthValue | HeightValue)) return FAILURE;
  if ((bitmask & HeightValue) == 0) height = width;
...
struct Param {
  ...
  const char
      *default_argv[MAX_DEFAULT_ARGS]; /* default arguments if none are given */
...
void usage(char *progname)
{
  ...
  printf(
      ...
      "    -m, --monitor <monitor>     What monitor to display in; -1 means\n"
      "                                the overall virtual display\n"
      ...
      "    --scroll-everywhere         enable scrolling outside of the "
      "scrollbars too\n"
      "    --drag-reorder [<bool>]     allow dragging icons to reorder them "
      "(default: true)\n"
      "    --drag-modifier <mod>       modifier that enables drag: alt "
      "(default),\n"
      "                                shift, ctrl, super, or none\n"
      "    --remember-icon-order [<bool>] persist icon order across tray "
      "restarts\n"
      "                                (default: true)\n"
      "    --icon-order-file <path>    path to the icon order state file\n"
      "                                (default: "
      "$XDG_STATE_HOME/stalonetray/order-<screen>)\n"
...
static struct Param *cmdline_match(const char *arg, const char **inline_val)
{
  ...
  *inline_val = NULL;
  ...
  for (p = params; p->parser != NULL; p++) {
    ...
    if (!is_short && !is_long) continue;

    if (p->max_argc == 0) {
      if ((is_short && arg[slen] == 0) || (is_long && arg[llen] == 0)) return p;
      continue;
    }

    if (is_short) {
      if (arg[slen] != '\0') *inline_val = arg + slen;
      return p;
    }
...
char *find_rc(const char *path_part1, const char *path_part2, const char *rc)
{
  ...
  if (path_part2 == NULL) {
    ...
    LOG_TRACE((
        "looking for config file in '%s/%s/%s'\n", path_part1, path_part2, rc));
...
#define PARSE_ERROR(reloading, lnum, fmt, ...)                                 \
  do {                                                                         \
    if (reloading) {                                                           \
      LOG_ERROR((                                                              \
          "reload: %s:%d: " fmt, settings.config_fname, lnum, ##__VA_ARGS__)); \
    } else {                                                                   \
...
static int parse_rc_line(char *buf, int lnum, int reloading)
{
  ...
  if (!get_args(buf, &argc, &argv)) {
    ...
    if (match == NULL) {
      PARSE_ERROR(
          reloading, lnum, "unrecognized rc file keyword \"%s\".\n", argv[0]);
      rc = FAILURE;
...

      {
        ...
        for (int i = 0; i < p_argc; i++) {
          int an = snprintf(argbuf + apos, sizeof(argbuf) - apos, "%s\"%s\"",
              i > 0 ? ", " : "", p_argv[i]);
          if (an < 0 || (size_t)(apos + an) >= sizeof(argbuf)) break;
          apos += an;
        }
        LOG_TRACE(("rc: param \"%s\", args [%s]\n", match->rc_name, argbuf));
      }

      if (!match->parser(p_argc, p_argv, match->struct_offset, False)) {
        PARSE_ERROR(
            reloading, lnum, "could not parse argument for \"%s\".\n", argv[0]);
        rc = FAILURE;
      }
    }
  }
...
int parse_rc(void)
{
  ...
  if (settings.config_fname == NULL)
    settings.config_fname = find_rc(getenv("HOME"), NULL, "." STALONETRAY_RC);
  if (settings.config_fname == NULL)
    settings.config_fname =
        find_rc(getenv("XDG_CONFIG_HOME"), NULL, STALONETRAY_RC);
  if (settings.config_fname == NULL)
    settings.config_fname = find_rc(getenv("HOME"), ".config", STALONETRAY_RC);
...
void interpret_settings(void)
{
  ...
  if (settings.scrollbars_highlight_color_str != NULL) {
    if (!x11_parse_color(tray_data.dpy, settings.scrollbars_highlight_color_str,
  ...
  if (tray_data.dpy != NULL) {
    if (x11_parse_color(tray_data.dpy, settings.bg_color_str, &tmp_color))
      ...
      LOG_ERROR(("reload: could not parse background color \"%s\"\n",
          settings.bg_color_str));
    if (settings.tint_level
        && x11_parse_color(tray_data.dpy, settings.tint_color_str, &tmp_color))
      settings.tint_color = tmp_color;
    if (settings.scrollbars_highlight_color_str != NULL
        && x11_parse_color(
            tray_data.dpy, settings.scrollbars_highlight_color_str, &tmp_color))
