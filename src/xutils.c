    ...
    unsigned int button, unsigned int state, int x, int y)
{
  ...
  XGetGeometry(dpy, dst, &dst_root, &idummy, &idummy, &udummy, &udummy, &udummy,
      &udummy);
...
int x11_send_expose(Display *, Window dst, int x, int y, int width, int height)
{
  XEvent xe;
  int rc;
  xe.type = Expose;
    ...
    Display *dpy, Window top, int *x, int *y, int depth)
{
  ...
  for (d = 1; d != depth && cur != old && old != None; d++) {
    ...
    for (c = nchildren - 1; c >= 0; c--) {
      ...
      if (xwa.x + bx <= *x && *x < xwa.x + xwa.width + bx && xwa.y + by <= *y
          && *y < xwa.y + xwa.height + by) {
        cur = children[c];
        bx += xwa.x;
        by += xwa.y;
        break;
